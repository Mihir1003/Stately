...
instance XPrint Expr where
  prt i' = \case
   E action expr -> P.concat $  elemFun i' "Expr" "E" : prt (i'+1) action : prt (i'+1) expr : [[P.replicate (i'+i') ' ' ++ endtag "E" "Expr"]]
   EAction action -> P.concat $  elemFun i' "Expr" "EAction" : prt (i'+1) action : [[P.replicate (i'+i') ' ' ++ endtag "EAction" "Expr"]]
   EVal val -> P.concat $  elemFun i' "Expr" "EVal" : prt (i'+1) val : [[P.replicate (i'+i') ' ' ++ endtag "EVal" "Expr"]]
...
instance XPrint Action where
  prt i' = \case
   ...
   ATask val0 val -> P.concat $  elemFun i' "Action" "ATask" : prt (i'+1) val0 : prt (i'+1) val : [[P.replicate (i'+i') ' ' ++ endtag "ATask" "Action"]]
