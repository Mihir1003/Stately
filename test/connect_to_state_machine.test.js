...
test("connect to state machine", async () => {
    const client = new client_sfn_1.SFNClient({ region: "us-east-1", endpoint: "http://localhost:8083" });
    try {
        let data = await client.send(new client_sfn_1.CreateStateMachineCommand({
            name: "test",
            definition: "{\
      ...
      \"States\": {\
        ...
        await client.send(new client_sfn_1.StartExecutionCommand({
...
afterAll(() => {
    const client = new client_sfn_1.SFNClient({ region: "us-east-1", endpoint: "http://localhost:8083" });
    client.send(new client_sfn_1.DeleteStateMachineCommand({
        stateMachineArn: "arn:aws:states:us-east-1:123456789012:stateMachine:test"
    }))
        // fail test if error
        .catch((err) => {
